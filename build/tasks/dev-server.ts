  ...
  return new Promise((resolve) => {
    ...
    if (typeof webpackConfigRenderer.entry === 'object') {
      Object.keys(webpackConfigRenderer.entry).forEach((name) => {
        if (!webpackConfigRenderer.entry)
          throw new Error('webpackConfigRenderer.entry');
      ...
      webpackConfigRenderer.entry = [
        ...hotClient,
        webpackConfigRenderer.entry,
      ] as string[];
    }
    WebpackDevServer.addDevServerEntrypoints(
      webpackConfigRenderer as Configuration,
      devServerOptions
    );
    ...
    rendererCompiler.hooks.done.tap('done', (stats) => {
      exConsole.success(
        `Server renderer start at ${chalk.magenta.underline(
          `http://${host}:${port}`
        )}`
      );
      resolve(stats);
